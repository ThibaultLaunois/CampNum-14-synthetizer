...
void ofApp::keyPressed(int key) {
	...
	if (keyToIndex.find(key) != keyToIndex.end()) {
		...
		// Vérifier si cette touche n'est pas déjà active
		bool alreadyActive = false;
		for (int i = 0; i < MAX_VOICES; i++) {
			if (voices[i].active && voices[i].keyIndex == index) {
				alreadyActive = true;
				break;
			}
		}
		...
		if (!alreadyActive) {
			...
			for (int i = 0; i < MAX_VOICES; i++) {
				if (!voices[i].active) {
					...
					voices[i].phase = 0.0f;
					voices[i].phaseAdder = (frequency / (float)sampleRate) * glm::two_pi<float>();
					voices[i].keyIndex = index;
					break;
				}
			}
		}
...
void ofApp::keyReleased(int key) {
	...
	if (keyToIndex.find(key) != keyToIndex.end()) {
		...
		// Désactiver la voix correspondante
		for (int i = 0; i < MAX_VOICES; i++) {
			if (voices[i].active && voices[i].keyIndex == index) {
				voices[i].active = false;
				voices[i].keyIndex = -1;
...
void ofApp::audioOut(ofSoundBuffer & buffer) {
	...
	for (size_t i = 0; i < buffer.getNumFrames(); i++) {
		...
		// Additionner toutes les voix actives
		for (int v = 0; v < MAX_VOICES; v++) {
			if (voices[v].active) {
				voices[v].phase += voices[v].phaseAdder;

				// Garder la phase dans la plage 0-2π
				while (voices[v].phase > glm::two_pi<float>()) {
					voices[v].phase -= glm::two_pi<float>();
				}

				sample += sin(voices[v].phase);
				activeCount++;
			}
		}
